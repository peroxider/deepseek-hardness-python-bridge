...
TypeScript 消费者需要静态类型的 `ctx.<service>` 方法，或者每个 Python dataclass 字段都必须成为独立且带文档的 Cordis 配置字段时，使用 `@deepseek-ai/dsh-python-bridge-codegen`。两条路径共用相同的 Python 运行时和线协议行为。
